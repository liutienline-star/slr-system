...
# --- Tab 1: 影像診斷錄入 ---
with tab_entry:
    st.markdown('<div class="warning-note">💡 專家提示：本診斷嚴禁編造頁碼。請根據「知識點」與「錯誤本質」執行精準指導。</div>', unsafe_allow_html=True)
    with st.container():
        ...
        if uploaded_file and st.button("🔍 執行深度事實診讀"):
            with st.spinner("掃描中..."):
                ...
                v_res = ai_engine.generate_content([
                    "你是一位教育診斷專家。請產出錯題題號、正確答案、知識點、錯誤本質分析及具體的修正指令。敘述需詳盡但禁止美化與情緒字眼，嚴禁編造頁碼。", img
                ])
        ...
        if st.button("🚀 同步數據至雲端戰情庫"):
            if stu_id and obs:
                with st.spinner("同步中..."):
                    diag = ai_engine.generate_content(f"根據錯題事實：{obs}。產出 200 字內補強指導，詳盡去美化，嚴禁頁碼。").text
    ...
    if hub_sheet:
        ...
        if raw_data:
            ...
            if not stu_df.empty:
                ...
                sel_mode = st.radio("選擇分析維度：", analysis_modes, horizontal=True)

                if sel_mode == "📡 跨科行為共性診斷":
                    if st.button(f"執行 {sel_stu} 跨科深度診斷"):
                        with st.spinner("比對中..."):
                            cross_context = "\n".join([f"{r['學科類別']}：{r['AI診斷與建議']}" for _, r in stu_df.head(10).iterrows()])
                            dispatch_res = ai_engine.generate_content(f"分析多科紀錄：{cross_context}。詳述底層邏輯漏洞，去美化，嚴禁頁碼。").text
                    ...
                    if st.button(f"生成 {target_sub} 詳盡補強指引"):
                        with st.spinner(f"分析 {target_sub} 趨勢..."):
                            ...
                            hunt_res = ai_engine.generate_content(f"針對 {target_sub} 紀錄：{history_context}。提供詳盡建議，禁止美化與頁碼。").text
                ...
                sel_sub_hist = st.selectbox("🔍 請選擇欲檢視的科目明細：", sub_list_hist, key="hist_filter")
                
                target_records = stu_df[stu_df['學科類別'] == sel_sub_hist]
                st.markdown(f'<div class="subject-header">📚 {sel_sub_hist} 診斷事實明細</div>', unsafe_allow_html=True)
